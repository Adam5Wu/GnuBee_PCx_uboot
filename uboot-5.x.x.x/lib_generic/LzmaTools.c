                  ...
                  unsigned char *inStream,  SizeT  length)
{
    ...
    debug("LZMA: Uncompressed size........... %d\n", outSizeFull);
    debug("LZMA: Compressed size............. %d\n", compressedSize);
    ...
    if (*uncompressedSize > 0) {
        ...
        if ((SizeT)inStream > (SizeT)outStream) {
            outProcessed = inStream - outStream;
        } else {
            outProcessed = DRAM_SIZE*0x10000 - ((SizeT)outStream - CFG_SDRAM_BASE);
        }
    }
    debug("LZMA: Decompression buffer........ %d\n", outProcessed);
